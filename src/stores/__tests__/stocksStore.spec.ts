...
describe('useStocksStore', () => {
    ...
    it('devrait charger les mouvements de produits avec succès', async () => {
        ...
        mockAxios.onGet(`${import.meta.env.VITE_API_URL}/stocks/${uniqueCode}/movements`).reply(200, {
    ...
    it('Devrait gérer les cas où uniqueCode est null ou undefined', async () => {
        ...
        mockAxios
            ...
            .reply(200, { data: mockStockMovements });

        await store.loadProductMovements('null');

        expect(store.loading).toBe(false); // Le chargement doit être terminé
        expect(store.error).toEqual('Request failed with status code 404');
        expect(store.stock_movements).not.toEqual(mockStockMovements); // Les données doivent correspondre.
    });
    ...
    it('doit gérer les erreurs lors du chargement des mouvements de produits', async () => {
        ...
        mockAxios.onGet(`${import.meta.env.VITE_API_URL}/stocks/${uniqueCode}/mouvements`).reply(404, {
    ...
    it('Devrait gérer les réponses vides ou malformées de l\'API', async () => {
        ...
        expect(store.loading).toBe(false); // Le chargement doit être terminé
        expect(store.error).toBe('Request failed with status code 404'); // Une erreur doit être capturée
    ...
    it('Devrait annuler la requête en cours si une nouvelle requête est lancée', async () => {
        ...
        mockAxios
            .onGet(`${import.meta.env.VITE_API_URL}/stocks/${uniqueCode}/mouvements`)
            .reply(() => new Promise((resolve) => setTimeout(() => resolve([200, { data: mockStockMovements }]), 200)));
        ...
        expect(store.stock_movements).toEqual([]); // Les données doivent correspondre
    });
});
